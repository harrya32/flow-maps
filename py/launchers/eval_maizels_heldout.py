"""Evaluate Maizels PCA50 D3 -> D8 models on held-out intermediate days."""
...
def main() -> None:
    ...
    parser.add_argument("--cfg_path", required=True, help="Config module, e.g. configs.maizels_pca50")
    ...
    cfg = cfg_module.get_config(args.slurm_id, args.dataset_location, args.out_dir)
    ...
    if x0_all.shape[0] == 0:
        raise RuntimeError(f"No Maizels source cells found for {source_time}.")
    if x1_all.shape[0] == 0:
        raise RuntimeError(f"No Maizels target cells found for {target_time}.")
    ...
    if not heldout_timepoints:
        raise RuntimeError("No Maizels held-out timepoints found.")
    ...
    for timepoint in heldout_timepoints:
        ...
        tau = float(np.clip((time_value - source_value) / (target_value - source_value), 0.0, 1.0))
        pred = np.asarray(
            pushforward_batch(
    ...
    csv_path = out_dir / "maizels_heldout_metrics.csv"
    ...
    np.savez(
        out_dir / "maizels_heldout_metrics.npz",
    ...
    plot_heldout_comparison(results, out_dir / "maizels_heldout_vs_estimated.png")
